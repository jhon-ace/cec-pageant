...
if(isset($_POST['submit_score']))
{
	...
	if($score <101){
	$update = "UPDATE score_card SET final_round = '$score',final_round_vote = '1' WHERE name_contestant = '$name_contestant' AND name_judge = '$judge'";
	...
	if($judge == 'judge1')
	{
		$sql_final_round_judge1 = "UPDATE final_round SET judge1 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_final_round_judge1);
	}
	elseif($judge == 'judge2')
	{
		$sql_final_round_judge2 = "UPDATE final_round SET judge2 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_final_round_judge2);
	}
	elseif($judge == 'judge3')
	{
		$sql_final_round_judge3 = "UPDATE final_round SET judge3 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_final_round_judge3);
	}
		elseif($judge == 'judge4')
	{
		$sql_final_round_judge4 = "UPDATE final_round SET judge4 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_final_round_judge4);
	}
		elseif($judge == 'judge5')
	{
		$sql_final_round_judge5 = "UPDATE final_round SET judge5 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_final_round_judge5);
	}
	}else{
		$message = "Input is greater than 100!";
      echo "<script type='text/javascript'>alert('$message'); window.location.assign('final_round.php')</script>";
	}
}
?>
<!doctype html>
...
<?php
    ...
    <main role="main">
      <section class="jumbotron jumbotron-fluid text-left py-5">
        <div class="container">
          <h2 class="jumbotron-heading">TOP 3 FINAL ROUND (100%) </h2>
						...
						for($b = 0 ; $b < $num_rows1 = mysqli_fetch_array($sql1) ; $b++ )
								{
									...
									if($name_judge == $judge)
									{
										$final_round_vote = $num_rows1['final_round_vote'];
										if($final_round_vote== 0)
										{
											echo "
											<div class = 'row'>
											<form method = 'post'>
