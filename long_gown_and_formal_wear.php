...
if(isset($_POST['submit_score']))
{
	...
	if($score <101){
	$update = "UPDATE score_card SET long_gown_formal_wear = '$score',long_gown_formal_wear_vote = '1' WHERE name_contestant = '$name_contestant' AND name_judge = '$judge'";
	...
	if($judge == 'judge1')
	{
		$sql_talent_night_judge1 = "UPDATE long_gown_formal_wear SET judge1 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge1);
	}
	elseif($judge == 'judge2')
	{
		$sql_talent_night_judge2 = "UPDATE long_gown_formal_wear SET judge2 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge2);
	}
	elseif($judge == 'judge3')
	{
		$sql_talent_night_judge3 = "UPDATE long_gown_formal_wear SET judge3 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge3);
	}
	elseif($judge == 'judge4')
	{
		$sql_talent_night_judge4 = "UPDATE long_gown_formal_wear SET judge4 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge4);
	}
	elseif($judge == 'judge5')
	{
		$sql_talent_night_judge5 = "UPDATE long_gown_formal_wear SET judge5 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge5);
	}
	}else{
		$message = "Input is greater than 100!";
      echo "<script type='text/javascript'>alert('$message'); window.location.assign('long_gown_and_formal_wear.php')</script>";
	}
}
?>
<!doctype html>
<html lang="en">
  <head>
    ...
    <title>Long Gown & Formal Wear</title>
...
<?php
		...
		$sql5 = mysqli_query($link,"SELECT *FROM score_card WHERE name_contestant = '$full_name' AND name_judge = '$judge'");
			...
			for($b = 0 ; $b < $num_rowst = mysqli_fetch_array($sql5) ; $b++ )
				...
				if($name_judge == $judge)
				{
					$long_gown = $num_rowst['long_gown_formal_wear_vote'];
						if($long_gown == 0)
							{
    ...
    <main role="main">
      <section class="jumbotron jumbotron-fluid text-left py-5">
        <div class="container">
          <h2 class="jumbotron-heading">Long Gown & Formal Wear (100%)</h2>
          <p>CRITERIA:<br>
            Beauty and Appeal (30)<br>
            Suitability to the Wearer (30)<br>
            Stage Presence (25)<br>  
            Audience Impact (15)</p>
        </div>
      </section>
						...
						for($b = 0 ; $b < $num_rows1 = mysqli_fetch_array($sql1) ; $b++ )
								{
									...
									if($name_judge == $judge)
									{
										$long_gown_vote = $num_rows1['long_gown_formal_wear_vote'];
										if($long_gown_vote == 0)
										{
											echo "
											<div class = 'row'>
