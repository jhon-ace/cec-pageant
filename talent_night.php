...
if($score <101){
	$update = "UPDATE score_card SET talent_portion = '$score',talent_portion_vote = '1' WHERE name_contestant = '$name_contestant' AND name_judge = '$judge'";
	...
	if($judge == 'judge1')
	{
		$sql_talent_night_judge1 = "UPDATE talent_portion SET judge1 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge1);
	}
	elseif($judge == 'judge2')
	{
		$sql_talent_night_judge2 = "UPDATE talent_portion SET judge2 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge2);
	}
	elseif($judge == 'judge3')
	{
		$sql_talent_night_judge3 = "UPDATE talent_portion SET judge3 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge3);
	}
}else{
	$message = "Input is greater than 100!";
	echo "<script type='text/javascript'>alert('$message'); window.location.assign('talent_night.php')</script>";
}
}
?>
<!doctype html>
...
<header>
	...
	<?php
		...
		$sql5 = mysqli_query($link,"SELECT *FROM score_card WHERE name_contestant = '$full_name' AND name_judge = '$judge'");
			...
			for($b = 0 ; $b < $num_rowst = mysqli_fetch_array($sql5) ; $b++ )
				...
				if($name_judge == $judge)
				{
					$talent_portion = $num_rowst['talent_portion_vote'];
						if($talent_portion == 0)
							{
	...
	<main role="main">
		<section class="jumbotron jumbotron-fluid text-left py-5">
			<div class="container">
				<h2 class="jumbotron-heading">Talent Portion (100%)</h2>
				<p>CRITERIA:<br>
					Entertainment Value (30)<br>
					Mastery (25)<br>
					Degree of Difficulty (20)<br>
					Stage Presence (15)<br>   
				Audience Impact (10)</p>
			</div>
		</section>
					...
					<div class="row">
						...
						for($a = 0 ; $a < $num_rows = mysqli_fetch_array($sql) ; $a++ )
						{
							...
							for($b = 0 ; $b < $num_rows1 = mysqli_fetch_array($sql1) ; $b++ )
							{
								...
								if($name_judge == $judge)
								{
									$talent_portion = $num_rows1['talent_portion_vote'];
									if($talent_portion == 0)
									{
										echo "
										<div class = 'row'>
