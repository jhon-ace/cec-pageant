...
if(isset($_POST['submit_score']))
{
	...
	if($score <101){
	$update = "UPDATE score_card SET preliminary_interview = '$score',preliminary_interview_vote = '1' WHERE name_contestant = '$name_contestant' AND name_judge = '$judge'";
	...
	if($judge == 'judge1')
	{
		$sql_talent_night_judge1 = "UPDATE preliminary_interview SET judge1 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge1);
	}
	elseif($judge == 'judge2')
	{
		$sql_talent_night_judge2 = "UPDATE preliminary_interview SET judge2 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge2);
	}
	elseif($judge == 'judge3')
	{
		$sql_talent_night_judge3 = "UPDATE preliminary_interview SET judge3 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge3);
	}

	elseif($judge == 'judge4')
	{
		$sql_talent_night_judge4 = "UPDATE preliminary_interview SET judge4 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge4);
	}

	elseif($judge == 'judge5')
	{
		$sql_talent_night_judge5 = "UPDATE preliminary_interview SET judge5 = '$score' WHERE name = '$name_contestant'";
		mysqli_query($link,$sql_talent_night_judge5);
	}
	}else{
		$message = "Input is greater than 100!";
      echo "<script type='text/javascript'>alert('$message'); window.location.assign('preliminary_interview.php')</script>";
	}
}
?>
<!doctype html>
<html lang="en">
  <head>
    ...
    <title>Preliminary Interview</title>
...
<?php
		...
		$sql5 = mysqli_query($link,"SELECT *FROM score_card WHERE name_contestant = '$full_name' AND name_judge = '$judge'");
			...
			for($b = 0 ; $b < $num_rowst = mysqli_fetch_array($sql5) ; $b++ )
				...
				if($name_judge == $judge)
				{
					$preliminary_interview_vote = $num_rowst['preliminary_interview_vote'];
						if($preliminary_interview_vote == 0)
							{
    ...
    <main role="main">
      <section class="jumbotron jumbotron-fluid text-left py-5">
        <div class="container">
          <h2 class="jumbotron-heading">Preliminary Interview (100%)</h2>
          <p>CRITERIA:<br>
            Delivery (30)<br>
            Relevance/Content/Impact (40)<br>
            Confidence (30)<br>
        </div>
      </section>
						...
						for($b = 0 ; $b < $num_rows1 = mysqli_fetch_array($sql1) ; $b++ )
								{
									...
									if($name_judge == $judge)
									{
										$interview = $num_rows1['preliminary_interview_vote'];
										if($interview == 0)
										{
											echo "
											<div class = 'row'>
											<form method = 'post'>
											<div class='input-group mb-3' style='margin-left:128%'>
